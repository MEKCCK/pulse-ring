...
- **[Kaleidux](https://github.com/Mjoyufull/Kaleidux)**（AGPL-3.0）—— 壁纸引擎：视频壁纸（GStreamer playbin/appsink）、50+ GLSL 切换动画库（gl-transitions）、mipmap 生成功能与过渡效果灵感

pulse-ring 采用 **AGPL-3.0-only**（与所参考项目 Folia/SPlayer/Kaleidux 的 AGPL-3.0-only 完全对齐；兼容原 GPL-3.0 条款）；引用来源均以概念/思路形式再实现于 Rust/JS/Cargo 代码中。

## 性能 / 帧率

- **帧率**：始终 30fps（`PULSE_RING_MAX_FPS=60` 可开 60fps），空闲动画（呼吸/自转/粒子/时钟）保持流畅；若想省电，可显式设置 `PULSE_RING_IDLE_FPS=15` 在静止时降低更新率。
